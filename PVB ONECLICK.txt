...
-- Переменная для отслеживания состояния посадки (защита от повторных циклов)
local isPlantingInProgress = false
...
-- Безопасное выполнение функции с повторными попытками
local function safeExecute(functionName, func, maxRetries, retryDelay)
    ...
    for attempt = 1, maxRetries do
        ...
        if success then
            ...
            if attempt < maxRetries then
                logError(functionName, result, "Попытка " .. attempt .. "/" .. maxRetries .. ", повтор через " .. retryDelay .. "с")
                wait(retryDelay)
            else
                logError(functionName, result, "Все попытки исчерпаны (" .. maxRetries .. ")")
                return false, result
            end
        end
    end
...
-- Async обертка для функций
local function asyncCall(func, callback)
    spawn(function()
        local success, result = pcall(func)
        if callback then
...
-- Получение отображаемого имени семени (ИСПРАВЛЕНО)
local function seedDisplayName(tool)
    -- Сначала пробуем получить из атрибута Plant
    local plantAttr = tool.GetAttribute and tool:GetAttribute("Plant")
    if plantAttr then
        if typeof(plantAttr) == "Instance" then 
            return plantAttr.Name 
        elseif type(plantAttr) == "string" and #plantAttr > 0 then 
            return plantAttr 
        end
    end
    
    -- Fallback: парсим из названия инструмента
    local name = tool.Name or ""
    local cleanName = name:gsub("[%[%]]",""):gsub("%s*[Ss]eed%s*",""):gsub("%s+$","")
    
    -- Дополнительная очистка для случаев типа "[x23] Sunflower Seed"
    cleanName = cleanName:gsub("^%[x%d+%]%s*", "") -- Убираем [x23] в начале
...
local SEED_PRIORITY = {
    ...
    -- СРЕДНИЕ (приоритет 3)
    ["Sunflower"] = 3,  -- ПОДСОЛНУХ ЛУЧШЕ ТЫКВЫ!
    
    -- ПЛОХИЕ (приоритет 4)
    ["Pumpkin"] = 4,    -- ТЫКВА ХУЖЕ ПОДСОЛНУХА!
    
    -- ПЛОХИЕ (приоритет 5) - ИСПРАВЛЕНО: Strawberry хуже чем Pumpkin
    ["Strawberry"] = 5,
    
    -- ХУДШИЕ (приоритет 6) - ИСПРАВЛЕНО: Cactus самый худший
    ["Cactus"] = 6
}
...
-- ==================== ИСПРАВЛЕННЫЙ ПОДСЧЕТ РАСТЕНИЙ ====================

-- Подсчет растений в ряду (ПОЛНОСТЬЮ ПЕРЕПИСАНО) - точный подсчет
local function countPlantsInRow(plot, rowNumber)
    local success, count = safeExecute("countPlantsInRow", function()
        local count = 0
        local row = plot:FindFirstChild("Rows") and plot.Rows:FindFirstChild(tostring(rowNumber))
        ...
        -- Создаем список позиций ячеек для быстрой проверки
        local cellPositions = {}
        for _, cell in pairs(cells) do
            local cellBase = getBase(cell)
            if cellBase then
                table.insert(cellPositions, cellBase.Position)
            end
        end
        
        -- Считаем выросшие растения по атрибуту Position
        local plants = plantsFolder(plot)
        if plants then
            for _, plant in pairs(plants:GetChildren()) do
                if plant:IsA("Model") then
                    -- ИСПОЛЬЗУЕМ АТРИБУТ POSITION ВМЕСТО ФИЗИЧЕСКОЙ ПОЗИЦИИ
                    local plantPosition = plant:GetAttribute("Position")
                    if plantPosition then
                        -- Проверяем тип атрибута Position
                        local positionVector
                        if typeof(plantPosition) == "CFrame" then
                            positionVector = plantPosition.Position
                        elseif typeof(plantPosition) == "Vector3" then
                            positionVector = plantPosition
                        else
                            print("[CountPlants] Неизвестный тип Position: " .. typeof(plantPosition))
                            positionVector = nil
                        end
                        
                        if positionVector then
                            -- Проверяем, в этом ли ряду растение по позиции
                            for _, cellPos in pairs(cellPositions) do
                                local distance = (positionVector - cellPos).Magnitude
                                if distance < 5 then -- Увеличиваем радиус для надежности
                                    count = count + 1
                                    print("[CountPlants] Найдено растение " .. plant.Name .. " в ряду " .. rowNumber .. " (позиция: " .. tostring(positionVector) .. ")")
                                    break
                                end
                            end
                        end
                    else
                        -- Fallback: используем физическую позицию если атрибут отсутствует
                        local plantPart = firstPart(plant)
                        if plantPart then
                            for _, cellPos in pairs(cellPositions) do
                                local distance = (plantPart.Position - cellPos).Magnitude
                                if distance < 3 then
                                    count = count + 1
                                    print("[CountPlants] Найдено растение " .. plant.Name .. " в ряду " .. rowNumber .. " (физическая позиция)")
                                    break
                                end
                            end
                        end
        ...
        -- Считаем ростки (используем физическую позицию для countdown)
            ...
            if countdownPlant.model then
                ...
                if plantPart then
                    for _, cellPos in pairs(cellPositions) do
                        ...
                        if distance < 3 then
                            count = count + 1
                            print("[CountPlants] Найден росток " .. countdownPlant.name .. " в ряду " .. rowNumber)
                            break
                        end
                    end
                end
        ...
        print("[CountPlants] Ряд " .. rowNumber .. ": найдено " .. count .. " растений")
        return count
    end, 2, 0.3)
...
-- Улучшенное удаление растения с sync ожиданием
local function removePlantByID(plantID, plantName, reason)
    local success, result = safeExecute("removePlantByID", function()
        -- Сохраняем позицию для проверки
        local plantPosition = nil
        local plants = plantsFolder(getCurrentPlot())
        if plants then
            for _, plant in pairs(plants:GetChildren()) do
                if getAnyIDFromInstance(plant) == plantID then
                    local pp = firstPart(plant)
                    if pp then
                        plantPosition = pp.Position
                    end
                    break
                end
            end
        ...
        -- SYNC ОЖИДАНИЕ удаления
        if plantPosition then
            if waitForPlantRemoval(getCurrentPlot(), plantPosition, 2) then
                print("[RemovePlant] ✓ Растение удалено: " .. plantName)
                return true
            else
                return false, "Растение не удалилось"
            end
        end
...
-- Поиск худшего растения в конкретном ряду (ИСПРАВЛЕНО) - использует физическую проверку
local function findWorstPlantInRow(plot, rowNumber)
    ...
    -- Проверяем выросшие растения по атрибуту Position
    local plants = plantsFolder(plot)
    if plants then
        for _, plant in pairs(plants:GetChildren()) do
            if plant:IsA("Model") then
                -- ИСПОЛЬЗУЕМ АТРИБУТ POSITION
                local plantPosition = plant:GetAttribute("Position")
                if plantPosition then
                    -- Проверяем тип атрибута Position
                    local positionVector
                    ...
                    if positionVector then
                        ...
                        for _, cell in pairs(cells) do
                            local cellBase = getBase(cell)
                            if cellBase then
                                local distance = (positionVector - cellBase.Position).Magnitude
                                if distance < 5 then -- Увеличиваем радиус
                                    local plantName = plant.Name
                    ...
                    -- Fallback: используем физическую позицию
                    local plantPart = firstPart(plant)
                    if plantPart then
                        for _, cell in pairs(cells) do
                            local cellBase = getBase(cell)
                            if cellBase then
                                local distance = (plantPart.Position - cellBase.Position).Magnitude
                                if distance < 3 then
                                    local plantName = plant.Name
                                    local priority = SEED_PRIORITY[plantName] or 999
...
-- Синхронное ожидание появления растения
local function waitForPlantSpawn(plot, cell, timeout)
    timeout = timeout or 3
    local cellBase = getBase(cell)
    if not cellBase then return false end
    ...
    return waitFor(function()
        ...
        if plants then
            for _, plant in pairs(plants:GetChildren()) do
                if plant:IsA("Model") then
                    local pp = firstPart(plant)
                    if pp and (pp.Position - cellBase.Position).Magnitude < 4 then
                        return true
                    end
                end
            end
        ...
        -- Проверяем ростки
        local countdownPlants = getCountdownPlants()
        for _, cp in ipairs(countdownPlants) do
            if cp.model then
                local pp = firstPart(cp.model)
                if pp and (pp.Position - cellBase.Position).Magnitude < 4 then
                    return true
                end
            end
...
-- Синхронное ожидание удаления растения
local function waitForPlantRemoval(plot, plantPosition, timeout)
    timeout = timeout or 3
    ...
    return waitFor(function()
        ...
        if plants then
            for _, plant in pairs(plants:GetChildren()) do
                if plant:IsA("Model") then
                    local pp = firstPart(plant)
                    if pp and (pp.Position - plantPosition).Magnitude < 2 then
                        return false -- Растение все еще существует
                    end
                end
            end
        end
        return true -- Растение удалено
    end, timeout, 0.1)
end

-- Улучшенная посадка на ячейку с sync ожиданием
local function plantOnCell(plot, cell, tool)
    local success, result = safeExecute("plantOnCell", function()
        local base = getBase(cell)
        if not base then 
            return false, "Базовая часть ячейки не найдена"
        end
        
        if not ensureHolding(tool) then 
            return false, "Не удалось взять инструмент в руку"
        end
        ...
        -- Отправляем запрос
        game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("PlaceItem"):FireServer(payload)
        
        -- SYNC ОЖИДАНИЕ появления растения
        if waitForPlantSpawn(plot, cell, 2) then
            print("[PlantOnCell] ✓ Растение появилось")
            return true
        end
        
        -- Повторная попытка
        print("[PlantOnCell] Повторная попытка...")
        game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("PlaceItem"):FireServer(payload)
        
        if waitForPlantSpawn(plot, cell, 2) then
            print("[PlantOnCell] ✓ Растение появилось (повтор)")
            return true
        end
        
        return false, "Растение не появилось после 2 попыток"
    end, 2, 0.5)
    
    if not success then
        print("[PlantOnCell] ✗ Ошибка: " .. tostring(result))
    end
...
-- ==================== НОВАЯ ФУНКЦИЯ: ПОЛУЧЕНИЕ ДОСТУПНЫХ ЯЧЕЕК В РЯДУ ====================
local function getAvailableCellsInRow(plot, row)
    local grass = row:FindFirstChild("Grass")
    if not grass then return {} end

    local cells = grass:GetChildren()
    -- Сортируем ячейки по позиции (от начала ряда, предполагая Z увеличивается вдоль ряда)
    table.sort(cells, function(a, b)
        return a.Position.Z < b.Position.Z
    end)
    ...
    for _, cell in ipairs(cells) do
        local isOccupied = false
        -- Проверяем выросшие растения
        local plants = plantsFolder(plot)
        if plants then
            for _, plant in pairs(plants:GetChildren()) do
                if plant:IsA("Model") then
                    local plantPosition = plant:GetAttribute("Position")
                    local positionVector = typeof(plantPosition) == "CFrame" and plantPosition.Position or typeof(plantPosition) == "Vector3" and plantPosition or nil
                    if positionVector then
                        local cellBase = getBase(cell)
                        if cellBase and (positionVector - cellBase.Position).Magnitude < 5 then
                            isOccupied = true
                            break
                        end
                    else
                        -- Fallback на физическую позицию
                        local plantPart = firstPart(plant)
                        if plantPart then
                            local cellBase = getBase(cell)
                            if cellBase and (plantPart.Position - cellBase.Position).Magnitude < 3 then
                                isOccupied = true
                                break
                            end
                        end
        ...
        for _, cp in ipairs(countdownPlants) do
            if cp.model then
                local plantPart = firstPart(cp.model)
                if plantPart then
                    local cellBase = getBase(cell)
                    if cellBase and (plantPart.Position - cellBase.Position).Magnitude < 3 then
                        isOccupied = true
...
-- ==================== ИСПРАВЛЕННАЯ ФУНКЦИЯ: УМНАЯ ПОСАДКА РАСТЕНИЙ ====================
-- Умная посадка растений (ПОЛНОСТЬЮ ПЕРЕПИСАНО) - садит в доступные ячейки
local function smartPlantSeeds(plot)
    local success, result = safeExecute("smartPlantSeeds", function()
        ...
        local plants = plantsFolder(plot)
        if plants then
            totalPlants = #plants:GetChildren()
        end
        totalPlants = totalPlants + #getCountdownPlants()
        
        if totalPlants >= 35 then
            print("[SmartPlant] Лимит достигнут (35/35)")
            return 0
        end
        ...
        for _, group in ipairs(seedGroups) do
            if #refreshUsable(group.items) > 0 then
                hasSeeds = true
                break
            end
        end
        ...
        if not hasSeeds then
            print("[SmartPlant] Нет доступных семян")
            return 0
        end
        
        -- Сортируем семена (лучшие первые)
        table.sort(seedGroups, function(a, b)
            return (a.priority or 999) < (b.priority or 999)
        end)
        
        -- Логируем доступные семена
        print("[SmartPlant] Доступные семена:")
        for _, group in ipairs(seedGroups) do
            local items = refreshUsable(group.items)
            if #items > 0 then
                print("[SmartPlant] - " .. group.name .. " (приоритет " .. (group.priority or 999) .. ", количество: " .. #items .. ")")
            end
        end
        
        -- Дополнительная отладка: показываем сгруппированные семена
        print("[SmartPlant] Сгруппированные семена:")
        for _, group in ipairs(seedGroups) do
            print("[SmartPlant] Группа: " .. group.name .. " (приоритет " .. (group.priority or 999) .. ", количество инструментов: " .. #group.items .. ")")
            for i, item in ipairs(group.items) do
                if i <= 3 then -- Показываем только первые 3
                    print("[SmartPlant]   - " .. item.Name .. " (usable: " .. tostring(stillUsable(item)) .. ")")
                end
            end
        end
        
        -- Дополнительная отладка: показываем все семена в инвентаре
        print("[SmartPlant] Все семена в инвентаре:")
        local backpack = LocalPlayer:WaitForChild("Backpack")
        for _, tool in pairs(backpack:GetChildren()) do
            if tool:IsA("Tool") and tool.Name:find("Seed") then
                local seedName = seedDisplayName(tool)
                local priority = SEED_PRIORITY[seedName] or 999
                local plantAttr = tool:GetAttribute("Plant")
                local stackCount = stackCount(tool)
                local usable = stillUsable(tool)
                print("[SmartPlant] - " .. tool.Name .. " -> " .. seedName .. " (приоритет " .. priority .. ", Plant attr: " .. tostring(plantAttr) .. ", stack: " .. stackCount .. ", usable: " .. tostring(usable) .. ")")
            end
        end
        ...
        local rows = plot:FindFirstChild("Rows")
        if not rows then
            logError("smartPlantSeeds", "Rows не найдены", nil)
            return 0
        end
        
        -- Собираем информацию о рядах
        local rowsInfo = {}
        for _, row in pairs(rows:GetChildren()) do
            local rowNumber = tonumber(row.Name)
            if rowNumber and row:GetAttribute("Enabled") then
                local plantsInRow = countPlantsInRow(plot, rowNumber)
                local availableCells = getAvailableCellsInRow(plot, row)
                
                table.insert(rowsInfo, {
                    rowNum = rowNumber,
                    row = row,
                    count = plantsInRow,
                    availableCells = availableCells
                })
                print("[SmartPlant] Ряд " .. rowNumber .. ": " .. plantsInRow .. "/5 растений, доступно ячеек: " .. #availableCells)
            end
        end
        
        -- Сортируем ряды (менее заполненные первые)
        table.sort(rowsInfo, function(a, b)
            return a.count < b.count
        end)
        
        -- ОСНОВНОЙ ЦИКЛ ПОСАДКИ
        for _, rowInfo in ipairs(rowsInfo) do
            local rowNumber = rowInfo.rowNum
            local currentCount = rowInfo.count
            local availableCells = rowInfo.availableCells
            
            print("[SmartPlant] Обработка ряда " .. rowNumber .. " (" .. currentCount .. "/5)")
            
            -- Получаем доступные ячейки в ряду
            local spaceLeft = 5 - currentCount
            
            if #availableCells < spaceLeft then
                spaceLeft = #availableCells
            end
            
            if spaceLeft <= 0 then
                -- Ряд заполнен, пытаемся заменить худшее
                local worstPlant = findWorstPlantInRow(plot, rowNumber)
                if not worstPlant then
                    print("[SmartPlant] Ряд " .. rowNumber .. " заполнен, худшее растение не найдено")
                    continue
                end
                
                -- Находим лучшее семя с приоритетом лучше худшего растения
                local bestSeed = nil
                local bestGroup = nil
                for _, group in ipairs(seedGroups) do
                    local items = refreshUsable(group.items)
                    if #items > 0 and group.priority < worstPlant.priority then
                        bestSeed = items[1]
                        bestGroup = group
                        break
                    end
                ...
                if bestSeed then
                    print("[SmartPlant] Замена " .. worstPlant.name .. " (приоритет " .. worstPlant.priority .. ") на " .. bestGroup.name .. " (приоритет " .. bestGroup.priority .. ")")
                    local removeSuccess = removePlantByID(worstPlant.id, worstPlant.name, "Замена на лучшее")
                    if removeSuccess then
                        wait(1.5)  -- Ожидание удаления
                        -- Обновляем доступные ячейки после удаления
                        availableCells = getAvailableCellsInRow(plot, rowInfo.row)
                        if #availableCells > 0 then
                            if plantOnCell(plot, availableCells[1], bestSeed) then
                                plantedCount = plantedCount + 1
                                bestGroup.items = refreshUsable(bestGroup.items)
                            end
                        end
                    end
                else
                    print("[SmartPlant] Нет лучших семян для замены в ряду " .. rowNumber)
                end
            else
                -- Ряд имеет место, сажаем в доступные ячейки
                for i = 1, spaceLeft do
                    ...
                    if not bestSeed then
                        print("[SmartPlant] Нет больше семян")
                        break
                    end
                    
                    -- Сажаем в следующую доступную ячейку
                    local cell = availableCells[i]
                    if plantOnCell(plot, cell, bestSeed) then
                        ...
                        break  -- Если ошибка, прерываем для этого ряда
                    end
                    
                    if plantedCount >= CONFIG.MAX_PLANTS_PER_CYCLE then
                        break
                    end
                end
            end
            ...
            if plantedCount >= CONFIG.MAX_PLANTS_PER_CYCLE then
                print("[SmartPlant] Достигнут лимит посадок за цикл")
                break
            end
        end
        
        print("[SmartPlant] Цикл завершен: посажено " .. plantedCount .. " растений")
        return plantedCount
    end, 2, 1)
...
-- ==================== ASYNC СИСТЕМА ПОСАДКИ ====================

-- Главная функция умной системы посадки (УЛУЧШЕННАЯ)
local function smartPlantingSystem()
    local success, result = safeExecute("smartPlantingSystem", function()
        local plots = findPlotsRoot()
        if not plots then
            return false, "Plots не найдены"
        end
        
        local myPlot = pickMyPlot(plots)
        if not myPlot then
            return false, "Plot не выбран"
        end
        ...
        if totalPlants >= 35 then
            return false, "Лимит достигнут (35/35)"
        end
        
        print("[SmartSystem] Запуск системы посадки (растений: " .. totalPlants .. "/35)")
        
        -- Удаляем избыточные растения
        local removedCount = smartRemovePlants(myPlot)
        if removedCount > 0 then
            print("[SmartSystem] Удалено избыточных растений: " .. removedCount)
            wait(2)
        end
        
        -- Запускаем посадку
        local plantedCount = smartPlantSeeds(myPlot)
        print("[SmartSystem] Посажено растений: " .. plantedCount)
...
-- Асинхронная система посадки (не блокирует основной поток)
local function asyncPlantingSystem()
    -- Защита от повторных циклов посадки
    if isPlantingInProgress then
        print("[AsyncPlant] Посадка уже выполняется, пропускаем цикл")
        return
    end
    
    isPlantingInProgress = true
    print("[AsyncPlant] Запуск системы посадки...")
    
    -- Используем spawn для асинхронного выполнения
    spawn(function()
        local success, result = pcall(function()
            return smartPlantingSystem()
        end)
        
        isPlantingInProgress = false  -- Сбрасываем флаг после завершения
        
        if success then
            print("[AsyncPlant] Система посадки завершена успешно")
        else
            print("[AsyncPlant] Ошибка системы посадки: " .. tostring(result))
        end
    end)
end
...
local function main()
    ...
    -- Основной цикл авто-посадки семян (ASYNC)
    spawn(function()
